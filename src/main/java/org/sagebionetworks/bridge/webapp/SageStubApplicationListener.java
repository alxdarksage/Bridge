...
public class SageStubApplicationListener implements ApplicationListener<ContextRefreshedEvent> {
		...
		if (object instanceof SageServicesStub) {
			try {
				...
				for (Specification spec: specResolver.getAllSpecifications()) {
					...
					if (spec.getName().equals("Complete Blood Count Tracker")) {
						trackerId = descriptor.getId();
						specification = specResolver.getSpecification(descriptor.getName());
					}
	...
	private void createCBCRecords(BridgeClient client, String trackerId, Specification spec, String... values) {
		try {
