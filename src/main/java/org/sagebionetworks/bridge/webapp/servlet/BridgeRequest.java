...
/**
 * A convenience wrapper to hide some of the ick of casting and using 
 * map keys when working with request and session objects.
 * 
 * @author alxdark
 *
 */
public class BridgeRequest extends HttpServletRequestWrapper {
    ...
    // Newer versions of Spring include a flash feature (a la Rails), but we don't 
    ...
    public void setOrigin(String origin) {
        if (origin == null) {
            this.request.removeAttribute(ORIGIN_KEY);
        } else {
            this.request.setAttribute(ORIGIN_KEY, origin);    
        }
    }
    public String getOrigin() {
        return (String)this.request.getAttribute(ORIGIN_KEY);
    }
    */
    ...
    /**
     * Return the root cause throwable of a request that has been routed to the global
     * servlet error page (as defined in web.xml).
     * 
