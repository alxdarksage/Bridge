...
/**
 * [This file is copied directly from Synapse Web Services.]
 * 
 * This filter redirects traffic received via http to https - kinda!
 * 
 * This is a little tricky. The load balancer actually does all https
 * communication with clients and talks to the protal-servlets using http:
 * 
 ...
 * 
 * Q: So if we want to re-direct clients from http to https how do we do so from
 * the portal? A: Look for the header: "X-Forwarded-Proto - a de facto standard
 * for identifying the originating protocol of an HTTP request, since a reverse
 * proxy (load balancer) may communicate with a web server using HTTP even if
 * the request to the reverse proxy is HTTPS"
 * 
 * @see http://en.wikipedia.org/wiki/List_of_HTTP_header_fields
 * 
 *      Therefore, this filter will redirect all http trafic with the following
 *      header:
 * 
...
public class SSLFilter implements Filter {
	...
	@Override
	public void doFilter(ServletRequest rqst, ServletResponse rspn, FilterChain chain) throws IOException,
			ServletException {
